...
const Chat: NextPageWithLayout = () => {
  ...
  return (
    <>
      ...
      <section className="relative flex h-full w-full flex-1 flex-col items-stretch overflow-hidden">
        <ScrollArea className="h-full">
          ...
          <div className="absolute bottom-0 left-0 mt-2.5 w-full bg-white dark:bg-zinc-900">
            ...
            <Button
              aria-label="Scroll to chat input"
              variant="ghost"
              className={cn(
                "absolute bottom-20 right-8 h-auto rounded-full p-0"
              )}
              onClick={() => {
                if (!chatRef.current) return
                console.log(chatRef.current.getBoundingClientRect().bottom)
            ...
            >
              <ArrowDownCircle
                ...
                aria-hidden="true"
              />
              <span className="sr-only">Scroll to chat input</span>
            </Button>
          </div>
        </ScrollArea>
      </section>
